...
def analyze_rtl_file(filepath,model):
    ...
    with open("phase1_evaluation.md", "a") as f:  # "a" = append, not overwrite
        f.write(f"\n\n## Model: {model}\n")
...
if __name__ == "__main__":
    ...
    for model in ["mistral", "tinyllama", "codellama"]:
        print(f"\nTesting model: {model}")
        result = analyze_rtl_file("test.sv", model)  # pass model in
